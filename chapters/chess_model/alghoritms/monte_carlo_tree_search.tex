...
\chapter[Algorytm Monte Carlo Tree Search {[\textit{Michał Lichtarski}]}]{Algorytm Monte Carlo Tree Search}
...
\caption[Etapy algorytmu Monte Carlo Tree Search]{Etapy algorytmu Monte Carlo Tree Search. Źródło: GeeksforGeeks, Monte Carlo Tree Search (MCTS) in Machine Learning}
\end{figure}
...
\end{axis}
\end{tikzpicture}
\caption[Składnik eksploracji]{Składnik eksploracji $\sqrt{\frac{\ln(N(s))}{N(s,a)}}$ dla $N(s)=50$}
...
\caption[Rozkład WDL]{Wykres rozkładu WDL podczas trwania partii szachowej. Źródło: Leela Chess Zero, Win-Draw-Loss evaluation}
\end{figure}
