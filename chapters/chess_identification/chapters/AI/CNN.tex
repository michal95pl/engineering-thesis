...
Finalna warstwa \texttt{Linear} mapuje 128 cech na 6 klas bierek. W procesie uczenia funkcja \texttt{LogSoftmax} (zaszyta w \texttt{CrossEntropyLoss}) wyznacza rozkład prawdopodobieństwa dla każdej klasy. Należy jednak zaznaczyć, że podczas identyfikacji bierek na szachownicy pomija się obliczanie prawdopodobieństw. Zamiast tego wybierany jest indeks o najwyższej wartości wyjściowej (logit) za pomocą operacji \texttt{argmax}.
...
Głównym problemem są cienie i odbicia światła na powierzchni tokenów. Ponieważ model operuje na dwuwymiarowych symbolach, silne odblaski mogą zniekształcić kontury piktogramu, prowadząc do błędnej klasyfikacji (np. mylenia Gońca ze Skoczkiem, jeśli odblask "odcina" fragment piktogramu). Aby zminimalizować ten efekt, w procesie uczenia wykorzystano techniki augmentacji z biblioteki \textit{Albumentations}, takie jak zmiany jasności, kontrastu oraz dodawanie szumu Gaussa \cite{Albumentations}, co ma na celu symulację trudnych warunków oświetleniowych panujących w rzeczywistym środowisku rozgrywki.
