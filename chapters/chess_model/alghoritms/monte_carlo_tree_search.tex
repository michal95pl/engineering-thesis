...
\section{WDL}
W opisywanym powyższym algorytmie PUCT zastosowanym w AlphaZero, wartość z value network po przepuszczeniu przez funkcję aktywacji sigmoid, określa prawdopodobieństwo wygranej w przedziale [-1, 1]. Jednakże istnieje lepszy sposób na oszacowanie wartości pozycji, który jest wykorzystywany w nowoczesnych silnikach takich jak LeelaChessZero \cite{lc0_wdl}. Zamiast przewidywać ogólną wartość stanu, sieć może przewidywać rozkład prawdopodobieństwa trzech możliwych wyników rozgrywki: wygrana, remis, przegrana. Taki sposób daje szerszy pogląd na obecną sytuację, gdzie 

\section{First play urgency}
W podstawowej wersji MCTS, podczas etapu selekcji priorytet wyboru mają nieodwiedzone węzły, które są wybierane losowo.  First play urgency (FPU) zmienia strategię wyboru takich węzłów, poprzez zastąpienie wartości eksloatacji wartością domyślną. Używając takiego mechanizmu pozbywamy się problemu dzielenia przez 0 podczas obliczania funkcji PUCT w nieodwiedzonym węźle.

Strategii FPU jest kilka. Ogólna zasada jest taka, że im większa jest wartość, tym bardziej faworyzowane są nieodwiedzone węzły, gdyż może ona przykrywać wartości eksploatacji odwiedzonych już węzłów. W przedstawianym algorytmie została użyta strategia "qFPU". Jest ona wartością oczekiwaną rodzica, czyli średnią sumy odwiedzonych węzłów oraz rodzica. W ten sposób nieodwiedzone węzły są faworyzowane, ale nie na tyle aby całkowicie przykryć już odwiedzone. W porównaniu do strategii ze stałą wartością, jest ona znacznie bardziej adaptacyjna i dynamicznie dostosowuje się do sytuacji \cite{Cazenave2021}.

Dla przykładu w sytuacji gdy obecna pozycja jest niekorzystna, w skutek czego FPU będzie miało wartość ujemną. Algortm będzie miał charakter mocno eksploracyjny do momentu znalezienia ruchu poprawiającego pozycję. Jeżeli zostanie on znaleziony, algorytm zacznie mocno go faworyzować. Dzięki temu podejściu algorytm szuka ucieczki z niekorzystnej pozycji, zamiast tracić czas na eksplorację jeszcze gorszych ruchów jak ma to miejsce w przypadku stałej, wysokiej wartości FPU. Natomiast jeżeli FPU będzie miało wysoką wartość dodatnią, to algorytm skupi się na eksploracji, tak aby mieć szanse znaleźć jeszcze lepszy ruch niż ten już znaleziony.