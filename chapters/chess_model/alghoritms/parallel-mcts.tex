...
\chapter[Zrównoleglenie MCTS {[\textit{Michał Lichtarski}]}]{Zrównoleglenie MCTS}
...
\caption[Naiwna metoda batchowania danych]{Naiwna metoda batchowania danych. Czerwone strzałki reprezentują wybraną najlepszą ścieżkę. Liść oznaczony numerem 6 został dodany do bufora i jest zablokowany. Na fioletowo została oznaczona ścieżka prowadząca do kolejnego liścia, który zostanie dodany do bufora.}
\end{figure}
...
\begin{description}
  ...
  \item[$G(s,a)$] - błąd estymacji wartości akcji $a$ w stanie $s$
...
\caption[WU-UCT]{WU-UCT. Czerwona strzałka przedstawia najlepszą ścieżkę, natomiast fioletowa przedstawia ścieżkę do kolejnego liścia dodawanego do bufora}
\end{figure}
...
Bardzo dobrze ilustruje to przypadek widoczny na rysunku 4.6, gdzie algorytm po inicjalizacji zaczyna wykonywać pierwszą selekcję zaczynając od korzenia. Naturalnie jest tylko jeden korzeń w drzewie, więc trzeba wówczas zapewnić, że tylko jeden proces będzie mógł go rozwinąć, a pozostałe będą musiały czekać. Obsługa takiego przypadku jest zaprezentowana na rysunku 4.7, gdzie algorytm od razu sprawdza czy są dostępne liście. Jeżeli nie, to czeka, albo przetwarza swój bufor niezależnie od rozmiaru. Wywołanie przetwarzania bufora w takim przypadku ma na celu uniknięcie sytuacji, gdzie procesy same się blokują czekając na odblokowanie liści w buforach.
...
Na rysunku 4.9 jest przedstawiony wykres porównawczy siedmiu gier rozegranych między opisywanym modelem z 9000 symulacjami, a Stockfishem. Większość partii dotrwała powyżej 60 ruchów. Wszystkie partie zakończyły się przegraną, co jest zrozumiałe biorąc pod uwagę siłę Stockfisha.
Na rysunku 4.10 jest ten sam wykres, ale dla 4500 symulacji. Łatwo można zauważyć, że mniejsza ilość symulacji przekłada się na gorsze wyniki algorytmu.

\begin{figure}[h]
\centering
\includegraphics[width=0.6\textwidth]{images/stockfish_9000.png}
\caption{Wykres przeżywalności MCTS względem Stockfish dla 9000 symulacji}
\end{figure}

\begin{figure}[h]
\centering
\includegraphics[width=0.6\textwidth]{images/mcts_4500.png}
\caption{Wykres przeżywalności MCTS względem Stockfish dla 4500 symulacji}
\end{figure}


\newpage

Rysunek 4.11 przedstawia wykres porównawczy siedmiu gier rozegranych między samą siecią neuronową, a Stockfishem. W tym przypadku sieć neuronowa nie korzysta z MCTS, a jedynie wybiera ruch na podstawie najwyższego prawdopodobieństwa zwracanego przez sieć. W porównaniu do poprzednich wykresów, widać pogorszenie się wyników.
\begin{figure}[h]
\centering
\includegraphics[width=0.6\textwidth]{images/stockfish_net.png}
\caption{Wykres przeżywalności sieci neuronowej względem Stockfish}
\end{figure}

Na rysunku 4.12 jest przedstawiony wykres wpływu ilości procesów na czas wykonania 6000 iteracji MCTS z buforem o rozmiarze 2. Testy zostały przeprowadzone na procesorze i7-11800H oraz GPU RTX 3060. Podobnie jak z rozmiarem bufora, wraz ze wzrostem ilości procesów różnica czasu wykonania maleje. W tym przypadku jest to spowodowane synchronizacją, gdzie wąskim gardłem jest brązowy blok przedstawiony na rysunku 4.7.
...
\begin{tikzpicture}
  ...
  \end{axis}
\end{tikzpicture}
\caption{Wpływ ilości procesów na czas wykonania 6000 iteracji}
\end{figure}
