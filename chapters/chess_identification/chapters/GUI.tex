...
Interfejs graficzny (GUI) systemu został zaprojektowany w celu zapewnienia intuicyjnej obsługi procesu kalibracji oraz wizualizacji stanu gry w czasie rzeczywistym. Aplikacja została zrealizowana w języku Python z wykorzystaniem biblioteki \texttt{tkinter}, a jej architektura opiera się na modelu maszyn stanowych, gdzie użytkownik przechodzi przez kolejne etapy konfiguracji.
...
Po wyborze kamery, system przechodzi do widoku \texttt{ColorCalibrationPage}. Jest to kluczowy etap dla modułu \texttt{BoardTransformation}. Użytkownik, klikając na podgląd wideo, wybiera próbki kolorów dla zielonych i czerwonych znaczników umieszczonych na rogach szachownicy. Wybrane wartości RGB są wizualizowane za pomocą obiektów \texttt{create\_oval} na płótnie \texttt{canvas}, co daje natychmiastową informację zwrotną o poprawności wyboru.

\section{Integracja obrazu wideo (VideoFrame)}
Najbardziej złożonym elementem GUI jest klasa \texttt{VideoFrame}, która dziedziczy po \texttt{threading.Thread}. Dzięki pracy w osobnym wątku, intensywne obliczenia związane z przetwarzaniem obrazu i inferencją sieci neuronowej nie blokują głównej pętli zdarzeń interfejsu (UI).
...
System GUI integruje się również z modułem \texttt{Communication}, który przesyła aktualny stan FEN do zewnętrznego serwera analizy. Wszystkie kluczowe zdarzenia, takie jak błędy połączenia, timeouty serwera czy wykryte ruchy, są rejestrowane przez klasę \texttt{Logger} i zapisywane w pliku dziennika, co ułatwia diagnostykę systemu.